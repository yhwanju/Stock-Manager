...
@client.tree.command(name="보유추가", description="보유종목을 추가하거나 업데이트합니다.")
@app_commands.describe(종목명="종목명", 티커="예: 195940.KQ", 수량="보유 수량", 평단="평균 단가")
async def add_holding_command(interaction: discord.Interaction, 종목명: str, 티커: str, 수량: int, 평단: float) -> None:
    await respond(interaction, bot_commands.add_holding, 종목명, 티커, 수량, 평단)
