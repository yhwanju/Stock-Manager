...
- `/관심추가 종목명`: 자동검색 후 관심종목 추가. 예: `/관심추가 풍산`
- `/관심추가직접 종목명 티커 테마`: 자동검색 실패 시 직접 입력. 예: `/관심추가직접 엔켐 348370.KQ 2차전지,ESS`
- `/관심삭제 종목명`: 관심종목 삭제
- `/관심목록`: 관심종목 목록
- `/종목매핑확인 종목명`: 자동검색 결과 확인. 예: `/종목매핑확인 풍산`
- `/관심테마수정 종목명 테마`: 관심/보유종목 테마 수정. 예: `/관심테마수정 풍산 원자재,방산`
- `/관심매수 종목명 수량 매수가`: 관심종목을 보유종목으로 이동하거나 자동 매핑으로 추가
- `/보유추가 종목명 수량 매수가`: 신규매수 또는 추가매수 반영, 평균단가 자동 재계산
- `/분할매도 종목명 수량 매도가`: 보유 수량 차감, 실현손익 기록
- `/전량매도 종목명 매도가`: 전체 수량 매도 처리, 보유목록에서 제거
...
1. `ticker_map.json`에서 종목명 또는 티커 검색
2. 없으면 `FinanceDataReader`의 `StockListing("KRX")`에서 한국 종목명 검색
3. 없으면 미국 티커처럼 보이는 입력값을 `yfinance`로 조회
4. 그래도 실패하면 `/관심추가직접 종목명 티커 테마` 사용

한국 종목은 KOSPI면 `.KS`, KOSDAQ이면 `.KQ`를 붙여 yfinance용 티커로 저장합니다. 미국 종목은 `NVDA`, `CRCL`, `WDC`처럼 티커를 입력하면 자동 조회합니다. 한글 미국주식명은 `ticker_map.json`에 보완해두면 됩니다.
